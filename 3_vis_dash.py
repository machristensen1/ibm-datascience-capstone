...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(values=launch_counts, 
                     names=launch_counts.index.tolist(),
                     title='Launch counts by site')
        return fig
    else:
        fig = px.pie(values=filtered_df['class'].value_counts(), 
                     names=filtered_df['class'].value_counts().index.tolist(), 
                     title='Total launch sessions for '+entered_site)
        return fig
# TASK 4:
...
def update_slider_chart(entered_site, slider_range):
    filtered_df = spacex_df[spacex_df['Launch Site'] == str(entered_site)]
